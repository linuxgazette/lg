...
<!--
     <xsl:call-template name="footer"> 
      ...
      <xsl:with-param name="status">Status: XSLT created 2003-12-11; modified 2004-01-27. Some of presentation style is attributed to Greg Elin's <a href="http:/fotonotes.net/">Fotonotes</a>. Stylesheet copyleft under GPL.</xsl:with-param>
     </xsl:call-template>
-->
    </div>
   </body>
...
-->
 ...
 <xsl:template name="genBox">
  ...
  <li><a href="#i{position()}" title="" class="box {$type}">
 ...
 <xsl:template match="image:hasPart/*|img:hasRegion/*">
  <table id="i{position()}" border="1" cellspacing="0" cellpadding="5">
   <caption>Region <xsl:value-of select="position()"/> (<xsl:value-of select="local-name()"/>)</caption>
