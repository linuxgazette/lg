                                    ...
                                    apr_int32_t flags)
{
    ...
    *bytes_sent = 0;
    int i=0;
    apr_size_t berhasil_dikirim;
    request_rec *r;
    apr_socket_t *s;
    struct tcp_info *info;
    apr_socklen_t tcp_info_length = sizeof(info);
    
    apr_size_t jumlah_byte_awal = file_bytes_left;*/
    int on;
    apr_size_t byte_terkirim; /* I add this as variable that keep the byte that just sent */
		
	

    do {
        apr_size_t tmplen = file_bytes_left;
		/* this getsockopt() will error at compiling, the same with setsockot() below is error in 
		   deferencing c->client_socket->socketdes */
		if (getsockopt(c->client_socket->socketdes, SOL_TCP, TCP_INFO, (void *)&info, &tcp_info_length) < 0) {
			return errno;
		}
		u_int32_t paket_baru_terkirim = info->tcpi_unacked;/* I add this too*/
		u_int32_t waktu_kirim = info->tcpi_last_data_sent; /* I add this too*/
		
            apr_time_t t_akhir= apr_time_now(); /* time to transfer packet.. I think.. */ 
		apr_time_t waktu_transfer= apr_time_sec(t_akhir)-apr_time_sec(waktu_awal_request); /* change the time to transfer to secon */
		
		if (i>0)
		{
			int throughput=(paket_baru_terkirim/waktu_kirim)*7.8125; /* this is what I still afraid of, I estimate the throughput 
													in this way (Kbit/s)*/
			/* Sorry,  the variables that I add here are in Indonesian languange.*/
			if (throughput<=14)
			{
				on=1;
			}
			else if (throughput>14 && throughput<=28)
			{
				on=2;
			}
			else if (throughput>28 && throughput<=33)
			{
				on=3;
			}
			else if (throughput>33 && throughput<=56)
			{
				on=4;
			}
			else if (throughput>56 && throughput<=128)
			{
				on=5;
			}
			else if (throughput>128 && throughput<=1024)
			{
				on=6;
			}
			else if (throughput>1024 && throughput<=4096)
			{
				on=7;
			}
			else if (throughput>4096 && throughput<=10240)
			{
				on=8;
			}
			setsockopt(c->client_socket->socketdes, SOL_SOCKET, SO_PRIORITY, (void *)&on, sizeof(int));/* this still error at compiling.
						The error are in deferencing c->client_socket->socketdes and 
                                    the SO_PRIORITY option is unknown*/
		}

        rv = apr_socket_sendfile(c->client_socket, fd, hdtr, &file_offset, &tmplen,
                                 flags);
        *bytes_sent += tmplen;
		byte_terkirim = tmplen;
