                                    ...
                                    apr_int32_t flags)
{
    ...
    *bytes_sent = 0;
    ...
    /*apr_time_t waktu_awal_request = r->request_time;
    apr_size_t jumlah_byte_awal = file_bytes_left;*/
    int on;
    apr_size_t byte_terkirim; /* Merupakan jumlah byte file yang sudah dikirim. */
	struct tcp_info *info;	
	struct sock *s;

    do {
        apr_size_t tmplen = file_bytes_left;
		if (getsockopt(s->socketdes, SOL_TCP, TCP_INFO, (void *)&info, &tcp_info_length) < 0) {
			return errno;
		}
		u_int32_t paket_baru_terkirim = info->tcpi_unacked;
		u_int32_t waktu_kirim = info->tcpi_last_data_sent;
		/*byte_terkirim = jumlah_byte_awal-file_bytes_left;
        apr_time_t t_akhir= apr_time_now(); /* waktu akhir transfer paket 
		apr_time_t waktu_transfer= apr_time_sec(t_akhir)-apr_time_sec(waktu_awal_request); /* mengubah waktu_transfer menjadi sekon*/
		
		if (i>0)
		{
			int throughput=(paket_baru_terkirim/waktu_kirim)*7.8125; /* estimasi throughput koneksi (Kbit/s)*/
			if (throughput<=14)
			{
				on=1;
			}
			...
			apr_socket_opt_set(c->client_socket, APR_SO_KEEPALIVE, on);
		}
